...
import { 
  ...
  Lock, Network, Zap, BadgeCheck, Award, SquareTerminal
} from "lucide-react";

import Navbar from "../components/Navbar";
import Footer from "../components/Footer";

// --- EXACT IMPORTS MAPPED TO YOUR SCREENSHOT ---
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 w-full md:w-auto">
          ...
          <div>
            ...
            <p className="text-zinc-500 font-medium text-xs md:text-sm mt-2 md:mt-1 max-w-xl leading-relaxed px-2 md:px-0">
              Identify legacy hardware bottlenecks and security vulnerabilities in your perimeter. Direct intake with our technicians—no sales scripts.
            </p>
          </div>
        </div>
        <Link 
...
// --- 2. ANIMATED CERTIFICATION BADGES ---
const CertificationBadges = () => {
  const certs = [
    { name: "Cisco CCNA", desc: "Network Routing & Switching", icon: <Network className="text-blue-400" size={24} />, logo: ciscoLogo },
    { name: "ICT WX/GX", desc: "Unified Access Control", icon: <ShieldCheck className="text-emerald-400" size={24} />, logo: ictProtegeLogo },
    { name: "Keyscan", desc: "Facility Security Integration", icon: <Lock className="text-yellow-400" size={24} />, logo: keyscanLogo },
    { name: "Avigilon", desc: "Advanced Video Analytics", icon: <Camera className="text-red-400" size={24} />, logo: avigilonLogo }
  ];

  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-6xl mx-auto mb-16 relative z-10">
      {certs.map((cert, i) => (
        <div key={i} className="group relative bg-zinc-900 border border-zinc-800 rounded-xl p-6 overflow-hidden hover:border-zinc-600 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_40px_-10px_rgba(250,204,21,0.15)] flex flex-col items-center text-center cursor-default">
          <div className="absolute inset-0 bg-gradient-to-b from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
          <div className="mb-4 w-full h-16 flex items-center justify-center rounded-full border border-zinc-800 bg-white p-3 transition-all duration-300 group-hover:scale-105 group-hover:border-zinc-500 relative">
            <img src={cert.logo} alt={cert.name} className="h-full w-full object-contain" />
          </div>
          <h4 className="text-white font-black uppercase tracking-wider text-sm md:text-base mb-1 relative z-10">
            {cert.name}
          </h4>
          <p className="text-zinc-500 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] relative z-10">
            {cert.desc}
          </p>
        </div>
      ))}
...
// --- 3. REVAMPED FIELD EXPERTISE SECTION ---
const ModernFieldExpertise = () => {
  return (
    <section className="py-20 md:py-32 bg-[#050608] relative overflow-hidden w-full border-y border-zinc-800 mt-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="max-w-3xl mb-16 text-center md:text-left mx-auto md:mx-0">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 mb-6 text-[10px] font-black tracking-[0.3em] text-yellow-400 border border-yellow-400/20 rounded-full uppercase bg-yellow-400/10">
            <BadgeCheck size={14} /> Field Authority
          </div>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-black uppercase tracking-tighter text-white leading-[1.1]">
            Certified Technicians For <span className="text-transparent bg-clip-text bg-gradient-to-r from-zinc-400 to-zinc-600">Modern Facilities</span>
          </h2>
          <p className="text-zinc-400 text-sm md:text-lg leading-relaxed mt-6 max-w-2xl">
            We don't just pull cables. We bring certified engineering logic to the physical layer. Our field technicians execute complex installations backed by hands-on deployment experience and rigorous industry credentials.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="group bg-zinc-900/50 backdrop-blur-sm border border-zinc-800 rounded-2xl p-8 hover:bg-zinc-900 transition-all duration-300">
            <div className="flex items-center gap-4 mb-6">
              <div className="p-3 rounded-lg bg-blue-500/10 text-blue-400 border border-blue-500/20">
                <SquareTerminal size={24} />
              </div>
              <h3 className="text-lg font-black uppercase tracking-widest text-white">Network-First Logic</h3>
            </div>
            <p className="text-zinc-400 text-sm leading-relaxed">
              Every camera mounted and every door controller wired is backed by practical CCNA switching, routing, and secure VLAN segmentation practices.
            </p>
          </div>

          <div className="group bg-zinc-900/50 backdrop-blur-sm border border-zinc-800 rounded-2xl p-8 hover:bg-zinc-900 transition-all duration-300">
            <div className="flex items-center gap-4 mb-6">
              <div className="p-3 rounded-lg bg-yellow-400/10 text-yellow-400 border border-yellow-400/20">
                <Zap size={24} />
              </div>
              <h3 className="text-lg font-black uppercase tracking-widest text-white">Active Deployment</h3>
            </div>
            <p className="text-zinc-400 text-sm leading-relaxed">
              Experienced installers available for the physical heavy lifting. We handle conduit, precision mounting, rigorous system testing, and clean closeout support.
            </p>
          </div>

          <div className="group bg-zinc-900/50 backdrop-blur-sm border border-zinc-800 rounded-2xl p-8 hover:bg-zinc-900 transition-all duration-300 md:col-span-1">
            <div className="flex items-center gap-4 mb-6">
              <div className="p-3 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                <Award size={24} />
              </div>
              <h3 className="text-lg font-black uppercase tracking-widest text-white">Commercial Scale</h3>
            </div>
            <p className="text-zinc-400 text-sm leading-relaxed">
              Years of field execution in environments where clean wiring, reliable hardware, and clear documentation matter just as much as the final result.
            </p>
          </div>

          <div className="md:col-span-3 mt-4 group relative overflow-hidden bg-gradient-to-r from-zinc-900 to-black border border-zinc-800 rounded-2xl p-8 md:p-10 flex flex-col md:flex-row items-center gap-8 justify-between hover:border-zinc-700 transition-colors">
            <div className="absolute top-0 right-0 w-64 h-64 bg-yellow-400/5 rounded-full blur-3xl group-hover:bg-yellow-400/10 transition-colors pointer-events-none" />
            <div className="relative z-10 flex-1">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-2 h-2 bg-yellow-400 rounded-full animate-pulse" />
                <span className="text-[10px] md:text-xs font-black uppercase tracking-[0.3em] text-yellow-400">
                  White-Label Field Availability
                </span>
              </div>
              <p className="text-zinc-300 text-sm md:text-base leading-relaxed max-w-3xl">
                We act as the on-site technical arm for General Contractors, MSPs, and Facility Managers. Get flexible field coverage, expert communication, and professional project delivery without carrying the overhead of full-time regional hires.
              </p>
            </div>
            <div className="relative z-10 shrink-0 w-full md:w-auto">
              <Link to="/Quote" className="inline-flex w-full md:w-auto items-center justify-center gap-2 bg-white text-black px-8 py-4 font-black uppercase tracking-[0.2em] text-[10px] hover:bg-yellow-400 transition-colors rounded-sm">
                Request Subcontractor Info <ChevronRight size={16} />
              </Link>
            </div>
          </div>
        </div>
...
const TechStack = () => {
  ...
  return (
    <div className="bg-zinc-50 py-12 md:py-16 border-y border-zinc-200 relative overflow-hidden w-full">
        ...
        <div className="animate-marquee whitespace-nowrap flex items-center">
          {marqueeBrands.map((brand, i) => (
            <div key={i} className="flex items-center gap-4 md:gap-8 mx-3 md:mx-6 cursor-default">
              <div className="w-1.5 h-1.5 bg-yellow-400 rotate-45 shrink-0 hidden md:block" />
              
              {/* FIXED CONTAINER: Uniform rounded rectangles giving images plenty of space to scale up */}
              <div className="h-16 w-36 md:h-20 md:w-44 flex items-center justify-center rounded-2xl border border-zinc-200 bg-white p-3 md:p-4 shadow-sm hover:border-yellow-400 transition-colors">
                <img 
                  src={brand.logo} 
                  alt={brand.name} 
    ...
    <section className="py-20 md:py-32 px-4 sm:px-6 max-w-7xl mx-auto w-full">
      <div className="mb-12 md:mb-16 border-l-4 border-yellow-400 pl-4 md:pl-6">
        <h2 className="text-4xl md:text-5xl lg:text-6xl font-black uppercase tracking-tighter text-zinc-900 leading-[1.1] md:leading-none">
          System <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 to-yellow-600 italic">Integration</span>
        </h2>
        <p className="text-zinc-500 font-medium mt-4 max-w-2xl text-sm md:text-lg">
          We install and maintain the infrastructure that keeps facilities secure and data moving. Clean wiring, certified networks, and reliable hardware.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 md:auto-rows-[300px]">
        <div className="md:col-span-2 bg-[#050608] rounded-2xl p-6 md:p-10 relative overflow-hidden group border border-zinc-800 shadow-2xl flex flex-col justify-end min-h-[320px] md:min-h-0">
          ...
          <div className="absolute top-6 left-6 md:top-8 md:left-8 flex items-center gap-2">
            <div className="w-2 h-2 bg-yellow-400 rounded-full animate-pulse" />
            <span className="text-yellow-400 font-mono text-[9px] md:text-[10px] uppercase tracking-widest font-black">Hardware Integration</span>
          </div>
          <div className="relative z-10 w-full md:w-3/4 mt-12 md:mt-0">
            <h3 className="text-2xl md:text-3xl font-black uppercase text-white mb-3 leading-tight">Tailored Deployments</h3>
            <p className="text-zinc-400 font-medium text-sm leading-relaxed">
              From edge computing devices to specialized surveillance servers. We build, mount, and integrate the hardware your facility needs to operate securely.
            </p>
          </div>
        </div>
        ...
        <div className="md:row-span-2 bg-yellow-400 rounded-2xl p-6 md:p-10 relative overflow-hidden group shadow-xl flex flex-col min-h-[280px] md:min-h-0">
          ...
          <p className="text-black/70 font-bold text-xs md:text-sm leading-relaxed mb-auto">
            Authorized installations for ICT WX/GX, Keyscan, and HID mobile credentials. Reliable door security and facility lockdowns.
          </p>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 md:p-10 relative overflow-hidden group shadow-xl min-h-[240px] md:min-h-0 flex flex-col">
          ...
          <h3 className="text-xl md:text-2xl font-black uppercase text-white mb-2 leading-tight">Video Security</h3>
          <p className="text-zinc-400 font-medium text-xs md:text-sm leading-relaxed relative z-10">
            Avigilon & Genetec camera deployments featuring clear optics, LPR, and reliable local or cloud storage.
          </p>
        </div>
      </div>
    </section>
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Professional installation of commercial CCTV, structured cabling, access control, and networking across Ontario." />
      </Helmet>
          ...
          <section className="bg-[#050608] pt-32 pb-24 md:pt-48 md:pb-40 px-4 sm:px-6 relative flex flex-col items-center text-center overflow-hidden w-full">
            ...
            <div className="max-w-6xl mx-auto relative z-10 animate-fade-in-up mt-8 md:mt-0 w-full flex flex-col items-center">
              <div className="inline-flex items-center gap-2 md:gap-3 px-3 py-1.5 md:px-4 md:py-2 mb-8 md:mb-10 text-[9px] md:text-[10px] font-black tracking-[0.2em] md:tracking-[0.3em] text-yellow-400 border border-yellow-400/20 rounded-full uppercase bg-yellow-400/5 shadow-[0_0_15px_rgba(250,204,21,0.1)]">
                <Terminal size={12} className="md:w-3.5 md:h-3.5 shrink-0" /> Installation & Deployment
              </div>
              ...
              <h1 className="text-[11vw] sm:text-[9vw] md:text-[7rem] lg:text-[8.5rem] font-black uppercase tracking-tighter leading-[1.1] md:leading-[0.85] mb-8 md:mb-10 text-white w-full">
                <span className="block mb-1 md:mb-0">Reliable</span>
              ...
              <p className="text-zinc-400 text-sm sm:text-base md:text-xl font-medium leading-relaxed max-w-2xl mx-auto mb-10 md:mb-12 px-2 md:px-0">
                We build and maintain the physical layer of your business. From enterprise surveillance and zero-latency networking to secure access control systems.
              </p>
          ...
          <section className="py-16 md:py-24 bg-zinc-900 border-y border-zinc-800 w-full">
            <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 md:gap-10 text-center">
               ...
               <div className="flex flex-col items-center gap-3">
                 ...
                 <span className="text-white font-black uppercase tracking-widest text-sm md:text-base">Reliable Connectivity</span>
                 <span className="text-zinc-500 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em]">Built for Uptime</span>
               </div>
               <div className="flex flex-col items-center gap-3">
                 <Cpu className="text-yellow-400 mb-1 md:mb-2 shrink-0" size={28} />
                 <span className="text-white font-black uppercase tracking-widest text-sm md:text-base">Certified Hardware</span>
                 <span className="text-zinc-500 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em]">Industry Standard</span>
               </div>
               <div className="flex flex-col items-center gap-3">
                 <Lock className="text-yellow-400 mb-1 md:mb-2 shrink-0" size={28} />
                 <span className="text-white font-black uppercase tracking-widest text-sm md:text-base">Secure Deployments</span>
                 <span className="text-zinc-500 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em]">Protected Networks</span>
               </div>
            </div>
          </section>
          ...
          <section className="py-24 md:py-40 bg-yellow-400 border-t-[12px] md:border-t-[16px] border-black text-center px-4 sm:px-6 w-full">
            <div className="max-w-4xl mx-auto flex flex-col items-center w-full">
               ...
               <p className="text-black/70 font-black tracking-[0.2em] md:tracking-[0.3em] mb-10 md:mb-12 text-[10px] md:text-xs uppercase px-2 text-center w-full">
                 Direct Technician Contact • No Middlemen
               </p>
